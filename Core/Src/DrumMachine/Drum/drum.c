...
uint8_t					UsbMidiRXBuf[MIDI_DATA_FS_MAX_PACKET_SIZE],UsbMidiPacket[MIDI_DATA_FS_MAX_PACKET_SIZE];
...
void application_set_usb(void)
{
	bB2xx_Set_USB_Ptrs(DrumMachineVar.midi_rx_buf,(uint8_t *)&DrumMachineVar.midi_rx_len);
}

void application_usb_callback(void)
{
	DrumMachineVar.midi_flag |= MIDI_RX_PACKET_READY;
}
...
void application_loop(void)
{
	...
	if (( DrumMachineVar.midi_flag & MIDI_RX_PACKET_READY ) == MIDI_RX_PACKET_READY)
	{
		DrumMachineVar.midipacket_len = DrumMachineVar.midi_rx_len;
		memcpy(DrumMachineVar.midi_packet,DrumMachineVar.midi_rx_buf,DrumMachineVar.midi_rx_len);
		UsbMidiParser();
		DrumMachineVar.midi_flag &= ~MIDI_RX_PACKET_READY;
	}
